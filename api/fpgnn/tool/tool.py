...
def load_data(input_data, args, from_file=False):
    if from_file:
        # 如果是從文件讀取
        ...
        # 如果是從 SMILES 列表讀取
        lines = [[smile] for smile in input_data]  # 每個 SMILES 變成一個單獨的列表

    # 剩下的處理邏輯保持不變
    data = [MoleData(line, args) for line in lines]
    data = MoleDataSet(data)
    ...
    print('There are ', now_data_len, ' smiles in total.')
    if fir_data_len - now_data_len > 0:
        print('There are ', fir_data_len, ' smiles first, but ', fir_data_len - now_data_len, ' smiles are invalid.')
