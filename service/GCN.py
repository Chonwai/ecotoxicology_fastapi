...
chiral_tag_vocab = {
    Chem.rdchem.ChiralType.values[i]: i
    for i in range(len(Chem.rdchem.ChiralType.values))
}
hybridization_vocab = {
    Chem.rdchem.HybridizationType.values[i]: i
    for i in range(len(Chem.rdchem.HybridizationType.values))
}
...
bond_type_vocab = {
    Chem.rdchem.BondType.values[i]: i for i in range(len(Chem.rdchem.BondType.values))
}
bond_dir_vocab = {
    Chem.rdchem.BondDir.values[i]: i for i in range(len(Chem.rdchem.BondDir.values))
}
bond_stereo_vocab = {
    Chem.rdchem.BondStereo.values[i]: i
    for i in range(len(Chem.rdchem.BondStereo.values))
}
...
class Smile2Graph(object):
    ...
    def get_atom_features(self, atom):
        ...
        return [
            ...
            ],  # returns our hybridization 杂交 全是0 可以舍
...
def load_model(model_type):
    ...
    model_path = os.path.join(current_dir, "model", f"{model_type}.pickle")
...
def predict_toxicity(model, smiles_list):
    result = []
